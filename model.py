...
class CAModel(nn.Module):
    ...
    ----------
    n_channels: int (16)
        Number of channels in teh grid
    
    hidden_channels: int
        Hidden Channels that are related to the pixelwise 1x1 concolution
        
    fire_rate: float
        Number between 0 and 1. The lower it is, the more likely it is for cells to be set to 0 during the 'stochastic_update' process
        
    device: torch.device
        Determines on what device we perform alll the computations.
    
    Attributes
    ----------
    update_module: nn.Sequential
        The only part of the ntwork containing trainable parameters. Composed of 1x1 convolution, ReLu, and 1x1 convolution

    filters: torch.Tensor
        Constant tensor of shape '(n_channels, 1, 3, 3)'. (Sobel and Identity)
    """
    def __init__(self, n_channels=16, hidden_channels = 128, fire_rate = .5, device = None):
        super().__init__()
        ...
        # Update Step (only trainable parameters)
    ...
    def perceive(self, x):
        ...
        This is the onlu place where we include information on the neighboring cells. However, we are not using any learnable parameters.
        
        Parameters
        ----------
        x : torch.Tensor
            Shape '(n_channels, n_channels, grid_size, grid_size)'
    ...
    def stochastic_update(x, fire_rate):
        """
        Run Pixel-wise dropout. Lets cells update in a more random matter rather than having a master "clock"
        ...
        fire_rate: float 
            Number between 0 and 1. The higher the more likely a given cell updates.
    ...
    def get_living_mask(x):
        ...
        -------
        torch.Tensor
            Shape '(n_samples, n_channels, grid_size, grid_size)' and the dtype is bool.
        """
        return (
